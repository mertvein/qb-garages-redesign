...
QBCore.Functions.CreateCallback('paramiver', function(source, cb, data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cashBalance = Player.PlayerData.money['cash']
    local bankBalance = Player.PlayerData.money['bank']
    if data == "100$" then
        if cashBalance >= 100 then
            Player.Functions.RemoveMoney('cash', 100)
            TriggerClientEvent('QBCore:Notify', src, 'Aracını farklı garajdan aldığın için 100$ kesinti yapıldı.', 'error')
            cb(true)
            return
        elseif bankBalance >= 100 then
            Player.Functions.RemoveMoney('bank', 100)
            TriggerClientEvent('QBCore:Notify', src, 'Aracını farklı garajdan aldığın için banka hesabından 100$ kesinti yapıldı.', 'error')
            cb(true)
            return
        else
        ...
        TriggerClientEvent('QBCore:Notify', src, 'Aracını kendi garajından aldığın için herhangi bir kesinti yapılmadı.', 'error')
        cb(true)
        return
    end
end)

--[[RegisterNetEvent('paramiver', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cashBalance = Player.PlayerData.money['cash']
    local bankBalance = Player.PlayerData.money['bank']

    if data.helal == "100$" then
        if cashBalance >= 100 then
            Player.Functions.RemoveMoney('cash', 100)
            TriggerClientEvent('QBCore:Notify', src, 'Aracını farklı garajdan aldığın için 100$ kesinti yapıldı.', 'error')
        elseif bankBalance >= 100 then
